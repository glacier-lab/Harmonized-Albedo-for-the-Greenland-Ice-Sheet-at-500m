...
def calculate_linear_trend_vectorized(y: xr.DataArray, min_n: int) -> tuple[xr.DataArray, xr.DataArray]:
    """OLS slope (albedo/year) and two-tailed p-value using vectorized xarray ops."""
...
def run_trend_analysis(monthly: xr.DataArray) -> xr.Dataset:
    print("Computing linear trend (vectorized OLS)...")
    linear_slope, linear_pvalue = calculate_linear_trend_vectorized(monthly, MIN_VALID_OBS)
